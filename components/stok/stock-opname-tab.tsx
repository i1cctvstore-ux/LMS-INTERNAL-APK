  ...
  if (!canAccess) {
    return (
      <div className="p-6 text-sm text-neutral-500">
        Menu Stock Opname hanya untuk Admin dan Super Admin.
      </div>
    );
  }
...
function OpnameDetail({
  ...
  if (sortKey) {
    rows = [...rows].sort((a: any, b: any) => {
      const av = sortKey === "checked" ? (computeStatus(a).checked ? 1 : 0)
        : sortKey === "selisih" ? (computeStatus(a).selisih ?? -Infinity)
        : sortKey === "real" ? (a.real ?? -Infinity)
        : a[sortKey];
      const bv = sortKey === "checked" ? (computeStatus(b).checked ? 1 : 0)
        : sortKey === "selisih" ? (computeStatus(b).selisih ?? -Infinity)
        : sortKey === "real" ? (b.real ?? -Infinity)
        : b[sortKey];
...
function Th({
  ...
  return (
    <th className="border border-neutral-200 p-0">
      <button
        onClick={() => onClick(k)}
