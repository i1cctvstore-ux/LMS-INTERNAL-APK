...
export function AddProjectDialog({ onAdd }: AddProjectDialogProps) {
  ...
  async function handleOpenChange(next: boolean) {
    ...
    if (next && branches.length === 0) {
      const supabase = createClient()
      const [branchRes, { data: authUser }] = await Promise.all([
      ...
      if (authUser?.user) {
        const { data: myProfile } = await supabase
          ...
          .single()
        if (myProfile?.branch_id) setBranchId(myProfile.branch_id)
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="flex max-h-[60vh] flex-col gap-4 overflow-y-auto">
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="project-branch">Cabang</Label>
            <Select
              value={branchId}
            ...
            {!isSuperAdmin && !branchId ? (
              <p className="text-xs text-destructive">
                Akun Anda belum di-assign ke cabang mana pun. Hubungi Super
                Admin dulu sebelum bisa membuat proyek.
              </p>
            ) : null}
          </div>
