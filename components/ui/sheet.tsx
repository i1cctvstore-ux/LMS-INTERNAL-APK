...
// Sheet = panel yang muncul dari tepi layar (bawah/kanan/kiri/atas), dibangun
// di atas primitive Dialog yang sama dengan components/ui/dialog.tsx — cuma
// beda animasi & posisi. Cocok dipakai untuk detail/aksi cepat di mobile.
...
const SIDE_CLASSES: Record<"top" | "right" | "bottom" | "left", string> = {
  ...
  bottom:
    "inset-x-0 bottom-0 h-auto max-h-[85vh] rounded-t-xl border-t data-open:slide-in-from-bottom data-closed:slide-out-to-bottom",
}
...
function SheetHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("flex flex-col gap-1.5 px-4 pt-2 sm:px-6", className)}
      {...props}
    />
  )
}
...
function SheetFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "mt-auto flex flex-col gap-2 px-4 pb-2 sm:flex-row sm:justify-end sm:px-6",
        className
      )}
      {...props}
    />
