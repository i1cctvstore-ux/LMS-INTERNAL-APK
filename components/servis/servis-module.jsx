...
export default function ServisModule({ currentUserId, currentUserRole, currentUserBranchId }) {
...
// ---------- main app ----------
function App({ branchId, currentUserId, isSuperAdmin, branchSwitcher }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800">
      ...
      <div className={isDesktopLayout ? "flex" : ""}>
      <Sidebar
        tab={tab}
        setTab={setTab}
        open={navOpen}
      ...
      <div className="flex-1 min-w-0">
        {!isDesktopLayout && (
          <div className="bg-slate-900 text-white px-4 py-3.5 sticky top-0 z-30 flex items-center gap-3">
            <button onClick={() => setNavOpen(true)} className="p-1 -ml-1 text-slate-300 hover:text-white"><Menu size={20} /></button>
            <div className="flex-1">
              <h1 className="font-semibold text-sm leading-tight">{currentNavLabel}</h1>
              <p className="text-[11px] text-slate-400">{saving ? "Menyimpan..." : "Tersimpan"}{loadError && " · gagal simpan"}</p>
            </div>
          </div>
        )}
        {isDesktopLayout && (
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-white sticky top-0 z-30">
            <h1 className="font-semibold text-lg text-slate-800">{currentNavLabel}</h1>
            <p className="text-xs text-slate-400">{saving ? "Menyimpan..." : "Tersimpan"}{loadError && " · gagal simpan, coba lagi"}</p>
          </div>
        )}
    ...
    <div className="max-w-2xl">
      {!canManage && (
        <div className="mb-4 p-3 rounded-2xl bg-slate-100 text-xs text-slate-500">
          Mode Admin biasa — hapus Brand/Supplier/Produk hanya bisa dilakukan Admin Pusat (ganti mode di sidebar). Data Customer boleh ditambah & diedit, hapus tetap khusus Admin Pusat.
        </div>
      )}
