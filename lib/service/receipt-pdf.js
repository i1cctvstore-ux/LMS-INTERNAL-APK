...
function buildBaseDoc({ branchInfo, title, subtitle }) {
  // Custom [lebar, tinggi] dalam mm -- lebar tetap selebar A4 landscape
  // (297mm) biar tabel tetap lega, tapi TINGGINYA dikecilin (dari 210mm
  // standar A4 jadi 148mm) biar gak nyisain ruang kosong gede di bawah
  // buat konten yang pendek (1 customer, dikit item). Kalau kontennya
  // panjang (banyak item), otomatis lanjut ke halaman baru sendiri --
  // ini bukan batasan, cuma ukuran default aja.
  const doc = new jsPDF({ unit: "mm", format: [297, 148] });
