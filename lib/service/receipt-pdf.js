...
// disamain semua) — di-hardcode by branch UUID (pola yang sama dipakai
// di seluruh lib/stok/*.ts) karena kolom branches.name di database
// gak konsisten/rapi buat ditampilin apa adanya (ada yang huruf kecil
// semua, ada yang formatnya beda-beda).
...
function buildBaseDoc({ branchInfo, title, subtitle, orientation = "landscape", skipCompanyName = false }) {
  ...
  // Blok alamat+HP (+ nama toko kalau skipCompanyName=false) — fungsi
  // terpisah biar bisa dipanggil SEBELUM judul (pola lama: Tanda Terima
  // & Pengambilan) ATAU SESUDAH judul (pola baru gabung: Invoice &
  // Surat Jalan, judulnya udah termasuk "I1 CCTV" jadi baris nama
  // toko-nya di-skip di sini, alamat+HP-nya pindah ke bawah judul).
  function drawCompanyBlock() {
    if (!(branchInfo && (branchInfo.address || branchInfo.phone || branchInfo.name))) return;
    if (!skipCompanyName) {
      doc.setFont("helvetica", "bold");
      doc.setFontSize(15);
      doc.text(branchDisplayName(branchInfo), pageWidth / 2, y, { align: "center" });
      y += 5;
    }
  ...
  if (!skipCompanyName) drawCompanyBlock();
...
function addInfoLines(doc, y, lines) {
  ...
  lines.forEach(([label, value]) => {
    ...
    doc.text(labelText, 12, y);
    // Posisi value dihitung OTOMATIS dari lebar teks label yang
    // sebenarnya (bukan jarak tetap) -- biar label panjang kayak
    // "Tanggal Kirim:" gak numpuk/tabrakan sama isinya kayak sebelumnya.
    const labelWidth = doc.getTextWidth(labelText);
    doc.setFont("helvetica", "normal");
    doc.text(String(value || "-"), 12 + labelWidth + 3, y);
    y += 6;
  });
  return y + 2;
}
...
function addFooterAndSignatures(doc, y, pageWidth, footerText, leftLabel, rightLabel) {
  if (footerText) {
    ...
    // Sengaja DIBATASI ke ~170mm (bukan pakai pageWidth - 20 yang di
    // kertas landscape bisa sampai 277mm) -- kalau dibiarin selebar
    // itu, paragraf pendek kayak footer ini gak akan pernah "kepaksa"
    // pindah baris, jadi keliatan 1 baris super panjang yang aneh.
    // Dibatasi segini biar tetap jadi beberapa baris rapi kayak
    // paragraf biasa, walau kertasnya lebar.
    const wrapWidth = Math.min(pageWidth - 20, 170);
    const lines = doc.splitTextToSize(footerText, wrapWidth);
    doc.text(lines, 10, y);
    doc.setTextColor(0);
    y += lines.length * 4 + 12;
  } else {
...
export function generateTandaTerimaPDF(items, branchInfo, orientation = "landscape") {
...
export function generateSuratJalanPDF(batch, items, branchInfo, orientation = "landscape") {
  const { doc, pageWidth, y: y0 } = buildBaseDoc({
    branchInfo,
    title: `${branchDisplayName(branchInfo)} - SURAT JALAN KE SUPPLIER`,
    subtitle: batch.kodeBatch,
    orientation,
    skipCompanyName: true,
  });
  const y = addInfoLines(doc, y0, [
  ...
  // 2026-09 -- kolom "Kelengkapan" ditambahkan. Sebelumnya SAMA SEKALI
  // tidak dicetak di Surat Jalan (padahal wajib diisi & sudah tercatat
  // sejak barang diterima dari customer, dan sudah tercetak di Tanda
  // Terima) -- jadi tidak ada bukti tertulis kelengkapan barang saat
  // dikirim ke supplier, risiko dispute soal dus/adaptor/bracket dst
  // yang hilang/tertukar di jalan.
  autoTable(doc, {
    startY: y,
    head: [["Brand / Produk", "SN", "Kelengkapan", "Kerusakan", "Jenis"]],
    body: items.map((c) => [
      `${c.brand} ${c.produk}`,
      c.snDiterima,
      c.kelengkapan || "-",
  ...
  const finalY = doc.lastAutoTable.finalY + 6;
  addFooterAndSignatures(doc, finalY, pageWidth, null, "Toko", "Supplier");
  doc.save(`surat-jalan-${batch.kodeBatch}.pdf`);
}

// ---------- 3. Tanda Terima Pengambilan ----------
export function generatePickupPDF(items, orientation = "landscape") {
  const customerName = items[0]?.customerName;
  const customerPhone = items[0]?.customerPhone;
...
// ---------- 4. Invoice ----------
export function generateInvoicePDF(data, branchInfo, orientation = "landscape") {
  const { doc, pageWidth, y: y0 } = buildBaseDoc({
    branchInfo,
    title: `${branchDisplayName(branchInfo)} - INVOICE`,
