...
export async function loadServiceData(branchId: string): Promise<ServiceData> {
  ...
  const [claimsRes, batchesRes, invoicesRes, setoranRes, sparepartsRes, productsRes, stockLogRes, settingsRes] =
    await Promise.all([
  ...
  const firstError = [claimsRes, batchesRes, invoicesRes, setoranRes, sparepartsRes, productsRes, stockLogRes, settingsRes]
    .map((r) => r.error)
    .find(Boolean)
  ...
  const suppliers: string[] = settingsRow?.suppliers ?? []
...
async function syncClaims(branchId: string, prev: Claim[], next: Claim[], userId?: string) {
  ...
  if (inserted.length)
    tasks.push(supabase.from('service_claims').insert(inserted.map((c) => claimToRow(c, branchId, userId))))
...
async function syncBatches(branchId: string, prev: Batch[], next: Batch[], userId?: string) {
  ...
  const tasks: Promise<{ error: any }>[] = []
  if (inserted.length)
    tasks.push(supabase.from('service_batches').insert(inserted.map((b) => batchToRow(b, branchId, userId))))
...
async function syncInvoices(branchId: string, prev: Invoice[], next: Invoice[], userId?: string) {
  ...
  const tasks: Promise<{ error: any }>[] = []
  if (inserted.length)
    tasks.push(supabase.from('service_invoices').insert(inserted.map((i) => invoiceToRow(i, branchId, userId))))
...
async function syncSetoran(branchId: string, prev: SetoranEntry[], next: SetoranEntry[], userId?: string) {
  ...
  const tasks: Promise<{ error: any }>[] = []
  if (inserted.length)
    tasks.push(supabase.from('service_setoran').insert(inserted.map((s) => setoranToRow(s, branchId, userId))))
...
async function syncSpareparts(branchId: string, prev: Sparepart[], next: Sparepart[]) {
  ...
  const tasks: Promise<{ error: any }>[] = []
  if (inserted.length)
    tasks.push(supabase.from('service_spareparts').insert(inserted.map((p) => sparepartToRow(p, branchId))))
  updated.forEach((p) =>
...
async function syncProducts(branchId: string, prev: ProductItem[], next: ProductItem[]) {
  ...
  const tasks: Promise<{ error: any }>[] = []
  if (inserted.length)
    tasks.push(supabase.from('service_products').insert(inserted.map((p) => productToRow(p, branchId))))
  updated.forEach((p) =>
...
async function syncStockLog(branchId: string, prev: StockLogEntry[], next: StockLogEntry[], userId?: string) {
  ...
  if (inserted.length)
    tasks.push(
      supabase.from('service_sparepart_stock_log').insert(inserted.map((e) => stockLogToRow(e, branchId, userId))),
    )
...
export async function saveServiceSettings(
  branchId: string,
  settings: { brands: string[]; suppliers: string[]; hiddenColumns: string[] },
) {
  const supabase = createClient()
...
export async function persistServiceData(
  ...
  if (patch.settings) {
    ...
    if (
      nextS.brands !== prevS.brands ||
      nextS.suppliers !== prevS.suppliers ||
      nextS.hiddenColumns !== prevS.hiddenColumns
    ) {
      tasks.push(
        saveServiceSettings(branchId, {
          brands: nextS.brands ?? prevS.brands,
          suppliers: nextS.suppliers ?? prevS.suppliers,
