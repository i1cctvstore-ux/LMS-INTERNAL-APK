...
// sparepartFromRow sekarang butuh qty dari LUAR (query terpisah, per
// cabang) — lihat pemakaiannya di loadServiceData.
function sparepartFromRow(row: any, qty: number): Sparepart {
  return { id: row.id, name: row.name, unit: row.unit || 'pcs', qty }
}

// Sparepart (jenis-nya: nama+satuan) sekarang katalog GLOBAL — qty
// TIDAK dikirim lewat sini lagi, itu jalan lewat setBranchSparepartQty().
function sparepartCatalogToRow(p: Partial<Sparepart>) {
...
async function setBranchSparepartQty(branchId: string, sparepartId: string, qty: number) {
  const supabase = createClient()
  const { error } = await supabase.rpc('set_branch_sparepart_qty', {
    p_branch_id: branchId,
    p_sparepart_id: sparepartId,
    p_qty: qty,
  })
...
// branch_id yang dikirim.
function productToRow(p: Partial<ProductItem>) {
  const row: Record<string, unknown> = {}
  if (p.id !== undefined) row.id = p.id
...
// Supplier sekarang katalog GLOBAL (bukan per-cabang lagi).
function supplierToRow(s: Partial<SupplierItem>) {
  const row: Record<string, unknown> = {}
  if (s.id !== undefined) row.id = s.id
...
export async function loadServiceData(branchId: string): Promise<ServiceData> {
  ...
  const brands: string[] = (brandsRes.data ?? []).map((r: any) => r.name)
...
async function syncSpareparts(branchId: string, prev: Sparepart[], next: Sparepart[]) {
  const supabase = createClient()
  const { inserted, updated, deletedIds } = diffById(prev, next)
  const dbTasks: Promise<{ error: any }>[] = []
  const qtyTasks: Promise<void>[] = []

  if (inserted.length) {
    dbTasks.push(...chunkedInsertTasks(supabase, 'service_spareparts', inserted.map((p) => sparepartCatalogToRow(p))))
    inserted.forEach((p) => qtyTasks.push(setBranchSparepartQty(branchId, p.id, p.qty)))
  }

  updated.forEach((p) => {
    const before = prev.find((x) => x.id === p.id)
    // Nama/satuan itu katalog GLOBAL, qty itu per-cabang — dua-duanya
    // dicek terpisah supaya cuma yang beneran berubah yang ditulis.
    if (!before || before.name !== p.name || before.unit !== p.unit) {
      dbTasks.push(supabase.from('service_spareparts').update(sparepartCatalogToRow(p)).eq('id', p.id))
    }
    if (!before || before.qty !== p.qty) {
      qtyTasks.push(setBranchSparepartQty(branchId, p.id, p.qty))
    }
  })

  if (deletedIds.length) dbTasks.push(supabase.from('service_spareparts').delete().in('id', deletedIds))
...
async function syncSuppliers(prev: SupplierItem[], next: SupplierItem[]) {
  const supabase = createClient()
  const { inserted, updated, deletedIds } = diffById(prev, next)
  const tasks: Promise<{ error: any }>[] = []
  if (inserted.length)
    tasks.push(...chunkedInsertTasks(supabase, 'service_suppliers', inserted.map((s) => supplierToRow(s))))
  updated.forEach((s) =>
    tasks.push(supabase.from('service_suppliers').update(supplierToRow(s)).eq('id', s.id)),
  )
  if (deletedIds.length) tasks.push(supabase.from('service_suppliers').delete().in('id', deletedIds))
  await runAndCheck(tasks)
}

// Brand sekarang tabel tersendiri (service_brands), disinkronkan lewat
// perbandingan NAMA (bukan id) karena di sisi JS brand cuma array string.
async function syncBrands(prevNames: string[], nextNames: string[]) {
  const supabase = createClient()
  const prevSet = new Set(prevNames)
  const nextSet = new Set(nextNames)
  const toAdd = nextNames.filter((n) => !prevSet.has(n))
  const toRemove = prevNames.filter((n) => !nextSet.has(n))
  const tasks: Promise<{ error: any }>[] = []
  if (toAdd.length) tasks.push(...chunkedInsertTasks(supabase, 'service_brands', toAdd.map((name) => ({ name }))))
  if (toRemove.length) tasks.push(supabase.from('service_brands').delete().in('name', toRemove))
  await runAndCheck(tasks)
}
...
export async function persistServiceData(
  ...
  if (patch.settings) {
    ...
    if (nextS.products && nextS.products !== prevS.products) {
      tasks.push(syncProducts(prevS.products, nextS.products))
    }
    if (nextS.supplierDetails && nextS.supplierDetails !== prevS.supplierDetails) {
      tasks.push(syncSuppliers(prevS.supplierDetails, nextS.supplierDetails))
    }
    ...
    if (nextS.brands && nextS.brands !== prevS.brands) {
      tasks.push(syncBrands(prevS.brands, nextS.brands))
    }
    if (nextS.hiddenColumns !== prevS.hiddenColumns) {
      tasks.push(saveServiceSettings(branchId, { hiddenColumns: nextS.hiddenColumns ?? prevS.hiddenColumns }))
    }
